...
import{ motion } from 'motion/react'              //hero
...
const Hero = () => { 
  ...
  const handleSearch = (e) => {
    e.preventDefault()
    navigate('/products?pickupLocation='+pickupLocation+'&pickupDate='+pickupDate+'&returnDate'+returnDate)
  }
...
<div className='flex flex-col items-start gap-2'>
  <select required value={pickupLocation} onChange={(e)=>setPickupLocation(e.target.value)}>
    <option value="">Pickup Loctaion</option>
     {cityList.map((city)=> <option key={city} value={city}>{city}</option>)}
  </select>
  <p className='px-1 text-sm text-gray-500'>{pickupLocation ? pickupLocation : 'please select loaction'}</p>
</div>
<div className='flex flex-col items-start gap-2'>
...
<div className='flex flex-col items-start gap-2'>
  ...
  Search
      ...
      <motion.img
        ...
        src={assets.main_banner} alt="banner" className='max-h-74' />
